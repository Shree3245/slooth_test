...
class MongoDBManager:
        ...
        try:
            if existing_client:
                ...
                if "created_at_index" not in self.db.leads.index_information():
                    self.db.leads.create_index("created_at")
                    
        except Exception as e:
            logger.error(f"Error initializing MongoDB client: {str(e)}")
            raise

    def store_lead(self, lead_data: Dict[str, Any], vector_embedding: List[float]) -> Optional[Dict[str, Any]]:
        """Store a new lead and return the stored document if successful."""
        try:
            # First check if URL already exists
            existing_lead = self.get_lead_by_url(lead_data["url"])
            if existing_lead:
                logger.info(f"Lead with URL already exists: {lead_data['url']}")
                return None
            ...
            if result.inserted_id:
                stored_doc = self.db.leads.find_one({"_id": result.inserted_id})
                logger.info(f"Successfully stored lead: {lead_data['title']}")
                return stored_doc
            return None
            
        except Exception as e:
            logger.error(f"Error storing lead in MongoDB: {str(e)}")
            return None
    ...
    def get_recent_leads(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            # Find leads sorted by created_at in descending order
            ...
            logger.info(f"Retrieved {len(leads)} recent leads from MongoDB")
            return leads
        except Exception as e:
